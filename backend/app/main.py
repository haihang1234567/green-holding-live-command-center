...
app=FastAPI(title=settings.app_name,version="3.6.0",lifespan=lifespan)
...
@app.get("/health")
def health():return {"status":"ok","app":settings.app_name,"version":"3.6.0","data_provider":settings.data_provider.upper(),"live_status_provider":settings.live_status_provider.upper(),"active_shop_count":len(settings.active_channel_ids),"polling_interval_seconds":settings.polling_interval_seconds,"live_history_lookback_days":settings.live_history_lookback_days,"architecture":"api-only-auto-monitor-with-live-history"}
