...
def _rows(db,sessions,snapshot_type):
    out=[]
    for s in sessions:
        snap=get_refund_snapshot(db,s.id,snapshot_type);m=session_metrics(s,snap);top=top_skus(db,s.id,1)
        out.append({"session_code":s.session_code,"date":s.started_at.date().isoformat(),"team":s.team.name,"channel":s.channel.name,"shift":"Ca sáng" if s.shift=="MORNING" else "Ca tối","gmv":m['gmv'],"net_revenue":int(snap.net_revenue) if snap else None,"orders":m['orders'],"refund_rate":float(snap.refund_cancel_rate) if snap else None,"refund_snapshot_available":snap is not None,"ads":m['ads_spend'],"roas":m['roas'],"aov":m['aov'],"gmv_per_hour":m['gmv_per_hour'],"top_sku":top[0]['product_name'] if top else "Chưa có dữ liệu"})
    return out

def _summary_data(db,date_from,date_to,team_id,channel_id,shift,snapshot_type):
    if snapshot_type not in SNAPSHOT_ORDER:snapshot_type="T3H"
    rows=_rows(db,_query(db,date_from,date_to,team_id,channel_id,shift),snapshot_type);complete=all(r['refund_snapshot_available'] for r in rows) and bool(rows)
    t={"gmv":sum(r['gmv'] for r in rows),"orders":sum(r['orders'] for r in rows),"ads":sum(r['ads'] for r in rows)}
    t['net_revenue']=sum(r['net_revenue'] for r in rows) if complete else None;t['aov']=round(t['gmv']/t['orders']) if t['orders'] else 0;t['roas']=round(t['gmv']/t['ads'],2) if t['ads'] else 0;t['refund_rate']=round((t['gmv']-t['net_revenue'])/t['gmv']*100,2) if t['gmv'] and complete else None;t['refund_snapshot_available']=complete
...
def summary(date_from:date|None=None,date_to:date|None=None,team_id:int|None=None,channel_id:int|None=None,shift:str|None=None,snapshot_type:str="T3H",db:Session=Depends(get_db)):
    return _summary_data(db,date_from,date_to,team_id,channel_id,shift,snapshot_type)
...
def excel(date_from:date|None=None,date_to:date|None=None,team_id:int|None=None,channel_id:int|None=None,shift:str|None=None,snapshot_type:str="T3H",db:Session=Depends(get_db)):
    data=_summary_data(db,date_from,date_to,team_id,channel_id,shift,snapshot_type);wb=Workbook();ws=wb.active;ws.title="Live Report";headers=["Session","Ngày","Team","Kênh","Ca","GMV","Net Revenue","Orders","Refund Rate %","Ads","ROAS","AOV","GMV/Hour","Top SKU"];ws.append(headers)
    ...
    for r in data['rows']:ws.append([r['session_code'],r['date'],r['team'],r['channel'],r['shift'],r['gmv'],r['net_revenue'] if r['net_revenue'] is not None else 'Chưa có dữ liệu',r['orders'],r['refund_rate'] if r['refund_rate'] is not None else 'Chưa có dữ liệu',r['ads'],r['roas'],r['aov'],r['gmv_per_hour'],r['top_sku']])
...
def pdf(date_from:date|None=None,date_to:date|None=None,team_id:int|None=None,channel_id:int|None=None,shift:str|None=None,snapshot_type:str="T3H",db:Session=Depends(get_db)):
    data=_summary_data(db,date_from,date_to,team_id,channel_id,shift,snapshot_type);font=_font();bio=BytesIO();c=canvas.Canvas(bio,pagesize=landscape(A4));w,h=landscape(A4)
    ...
    for r in data['rows']:
        ...
        vals=[r['date'],r['team'],r['channel'],f"{r['gmv']:,}",f"{r['net_revenue']:,}" if r['net_revenue'] is not None else 'N/A',str(r['orders']),f"{r['refund_rate']:.2f}" if r['refund_rate'] is not None else 'N/A',f"{r['ads']:,}",f"{r['roas']:.2f}"]
