...
def orders(
    ...
    query = select(Order).where(Order.channel_id.in_(settings.active_channel_ids))
    if session_id:
...
def attribution(
    ...
    if not session or session.channel_id not in settings.active_channel_ids:
...
def products(limit: int = Query(default=200, le=1000), db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    query = select(Product).where(Product.channel_id.in_(settings.active_channel_ids))
    if user.role == UserRole.TEAM.value:
...
def ads(session_id: int | None = None, limit: int = Query(default=200, le=1000), db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    query = select(AdsSnapshot).join(LiveSession,AdsSnapshot.live_session_id==LiveSession.id).where(LiveSession.channel_id.in_(settings.active_channel_ids))
    if session_id:
