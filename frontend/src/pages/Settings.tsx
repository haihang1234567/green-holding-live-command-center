...
export default function Settings(){
 ...
 return <>
  ...
  <section className="panel monitor-summary"><div className="panel-head"><div><span className="section-kicker">PRODUCTION FLOW</span><h3>{status.data_provider} / {status.live_status_provider}</h3></div><span className="mode-badge real">{status.polling_interval_seconds}s / lần</span></div><p className="muted">TikTok trả danh sách LIVE → lưu từng live_id vào database → cập nhật GMV và đơn hàng → giữ lịch sử sau khi phiên kết thúc.</p><div className="status-list"><Flag label="Lịch sử phiên LIVE" ok={status.live_history?.enabled} value={`Tự đồng bộ bù ${status.live_history?.lookback_days||30} ngày`}/></div></section>
